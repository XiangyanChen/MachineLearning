...
class Bayes:
    ...
    def train(self, dataSet, labels):   #训练样本 train
        ...
        n = len(self._vocab_list) #词条的特征数   feature num
    ...
    def predict(self, input_data):   #预测函数
        input_vec = self.set_of_word2vec(self._vocab_list, input_data)#测试样本的词条向量
        # np.multiply(self.p1Vect ,inputVec)
        p_positive = self._p_positive   #按照公式需要乘以p(y=1)的值，我们就以此为初始值
        p_negative = (1 - self._p_positive)
        for num in np.multiply(self._positive_vec ,input_vec):  #概率分布和词条向量进行相乘，得出p(x=xi|y=1)的概率，然后相乘
            if (num > 0):
                p_positive *= num
        for num in np.multiply(self._negative_vec ,input_vec):
            if (num > 0):
                p_negative *= num
