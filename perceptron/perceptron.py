...
class Perceptron:  # 感知机
    ...
    def train(self):
        m, n = np.shape(self._dataSet)  # m是行和n是列
        ...
        while flag != True:
            ...
            for i in range(m):  # 遍历样本 iterate samples
                ...
                if (self.sign(y) * self._labels[i] < 0):  # 说明是误分类了  it means this is wrong misclassification data
                    weights += self._labels[i] * self._dataSet[i]  # 更新权重
                    bias += self._labels[i]  # 更新偏置
